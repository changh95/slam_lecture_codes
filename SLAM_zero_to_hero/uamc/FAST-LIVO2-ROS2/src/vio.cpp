    ...
    if (grid_num[i] == TYPE_MAP)
    {
      ...
      for (int u = -patch_size_half; u <= patch_size_half; u++)
      {
        for (int v = -patch_size_half; v <= patch_size_half; v++)
        {
          ...
          int dep_row = v + int(pc[1]);
          int dep_col = u + int(pc[0]);
          if (dep_row < 0 || dep_row >= height || dep_col < 0 || dep_col >= width) continue;
          float depth = it[width * dep_row + dep_col];
...
void VIOManager::updateStateInverse(cv::Mat img, int level)
{
  ...
  for (int iteration = 0; iteration < max_iterations; iteration++)
  {
    ...
    int n_meas = 0;
    float error = 0.0;
    M3D Rwi_anchor(state->rot_end);
    V3D Pwi_anchor(state->pos_end);
    V3D Vwi_anchor = state->vel_end;
    P_wi_hat << SKEW_SYM_MATRX(Pwi_anchor);
    Rcw = Rci * Rwi_anchor.transpose();
    Pcw = -Rci * Rwi_anchor.transpose() * Pwi_anchor + Pci;

    const M3D Rcw_anchor = Rcw;
    const V3D Pcw_anchor = Pcw;
    const M3D P_wi_hat_anchor = P_wi_hat;
    const bool rs_active = rs_enable && rs_tr_readout > 0.0 && height > 0;
    const double rs_dt_per_row = rs_active ? rs_tr_readout / double(height) : 0.0;
    const double rs_half_h = 0.5 * double(height);
    ...
    for (int i = 0; i < total_points; i++)
    {
      ...
      M3D Rcw_f = Rcw_anchor;
      V3D Pcw_f = Pcw_anchor;
      M3D Rwi_f = Rwi_anchor;
      M3D P_wi_hat_f = P_wi_hat_anchor;

      if (rs_active)
      {
        V3D pf_anchor = Rcw_anchor * pt->pos_ + Pcw_anchor;
        V2D pc_anchor = cam->world2cam(pf_anchor);
        if (!new_frame_->cam_->isInFrame(pc_anchor.cast<int>(), border)) continue;

        double dt = (pc_anchor[1] - rs_half_h) * rs_dt_per_row;
        Rwi_f = Rwi_anchor * Exp(rs_omega_b, dt);
        V3D Pwi_f = Pwi_anchor + Vwi_anchor * dt + 0.5 * rs_accel_w * dt * dt;
        Rcw_f = Rci * Rwi_f.transpose();
        Pcw_f = -Rci * Rwi_f.transpose() * Pwi_f + Pci;
        P_wi_hat_f << SKEW_SYM_MATRX(Pwi_f);
      }

      V3D pf = Rcw_f * pt->pos_ + Pcw_f;
      pc = cam->world2cam(pf);
      ...
      for (int x = 0; x < patch_size; x++)
      {
        ...
        for (int y = 0; y < patch_size; ++y, img_ptr += scale)
        {
          ...
          JdR = J_dR * Rwi_f + J_dt * P_wi_hat_f * Rwi_f;
          Jdt = J_dt * Rwi_f;
...
void VIOManager::updateState(cv::Mat img, int level)
{
  ...
  double td_grad_converged = 0.0;
  double td_jac_sq_converged = 0.0;
  bool   td_iter_accepted = false;
  ...
  for (int iteration = 0; iteration < max_iterations; iteration++)
  {
    ...
    Rcw = Rci * Rwi_anchor.transpose();
    Pcw = -Rci * Rwi_anchor.transpose() * Pwi_anchor + Pci;
    Jdp_dt = Rci * Rwi_anchor.transpose();

    const M3D Rcw_anchor = Rcw;
    const V3D Pcw_anchor = Pcw;
    const M3D Jdp_dt_anchor = Jdp_dt;
    const bool rs_active = rs_enable && rs_tr_readout > 0.0 && height > 0;
    const double rs_dt_per_row = rs_active ? rs_tr_readout / double(height) : 0.0;
    const double rs_half_h = 0.5 * double(height);
    ...
    float error = 0.0;
    int n_meas = 0;
    double td_grad_sum = 0.0;
    double td_jac_sq_sum = 0.0;
    const V3D td_omega = rs_omega_b;
    const V3D td_vel   = state->vel_end;
    ...
    // omp_set_num_threads(desired_threads);
      ...
      #pragma omp parallel for reduction(+:error, n_meas, td_grad_sum, td_jac_sq_sum)
    #endif
    for (int i = 0; i < total_points; i++)
    {
      ...
      if (rs_active)
      {
        ...
        double dt = (pc_anchor[1] - rs_half_h) * rs_dt_per_row;
        M3D Rwi_f = Rwi_anchor * Exp(rs_omega_b, dt);
        V3D Pwi_f = Pwi_anchor + Vwi_anchor * dt + 0.5 * rs_accel_w * dt * dt;
        Rcw_f = Rci * Rwi_f.transpose();
        Pcw_f = -Rci * Rwi_f.transpose() * Pwi_f + Pci;
        Jdp_dt_f = Rci * Rwi_f.transpose();
      }
      ...
      for (int x = 0; x < patch_size; x++)
      {
        ...
        for (int y = 0; y < patch_size; ++y, img_ptr += scale)
        {
          ...
          if (td_estimate_enable)
          {
            // ∂res/∂td = JdR · ω_b + Jdt · v_w  (chain rule: state perturbation under td shift)
            double g_td = (JdR * td_omega)(0, 0) + (Jdt * td_vel)(0, 0);
            td_grad_sum  += g_td * res;
            td_jac_sq_sum += g_td * g_td;
          }
    ...
    if (error <= last_error)
    {
      ...
      td_grad_converged   = td_grad_sum;
      td_jac_sq_converged = td_jac_sq_sum;
      td_iter_accepted    = true;
  ...
  if (rs_debug_print)
  {
    printf("[ RS ] VIO level=%d  mean_photo_residual=%.4f  total_pts=%d\n",
           level, last_error, total_points);
  }

  // online td (img_time_offset) refinement — only on the finest pyramid level,
  // using the accumulators from the converged EKF iteration.
  td_step_pending = 0.0;
  td_last_grad_sum = 0.0;
  td_last_jac_sq_sum = 0.0;
  if (td_estimate_enable && level == 0 && td_iter_accepted)
  {
    td_last_grad_sum   = td_grad_converged;
    td_last_jac_sq_sum = td_jac_sq_converged;

    if (td_jac_sq_converged > td_min_jac_sq)
    {
      double dtd_raw  = -td_grad_converged / (td_jac_sq_converged + td_lambda);
      double dtd_step = td_step_alpha * dtd_raw;
      if (dtd_step >  td_step_cap) dtd_step =  td_step_cap;
      if (dtd_step < -td_step_cap) dtd_step = -td_step_cap;
      td_step_pending = dtd_step;
    }
  }
...
void VIOManager::dumpDataForColmap()
{
  ...
  cv::Mat img_rgb_undistort = img_rgb.clone();
