...
public:
  ...
  bool rolling_shutter_enable = false;
  double rs_tr_readout = 0.0;
  double rs_anchor_factor = 0.5;
  bool rs_debug_print = false;

  // online td estimator
  bool td_estimate_enable = false;
  double td_step_alpha = 0.1;
  double td_step_cap = 0.0005;
  double td_lambda = 100.0;
  double td_min_jac_sq = 1.0;

  // bracketing IMU samples around img_capture_time, captured during sync_packages
  // drainage so they survive measures.clear() before handleVIO runs.
  sensor_msgs::msg::Imu rs_imu_prev;
  sensor_msgs::msg::Imu rs_imu_next;
  double rs_imu_prev_t = -1.0;
  double rs_imu_next_t = -1.0;
...
#if defined(USE_LIVOX_ROS_DRIVER2) && defined(USE_LIVOX_INTERFACES)
  std::shared_ptr<rclcpp::SubscriptionBase> sub_pcl_livox_interfaces;
#endif
