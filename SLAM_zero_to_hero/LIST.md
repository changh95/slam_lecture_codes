...
| **Korea_drive** | `~/data/Korea_drive/KOREA_DRIVE/` (ROS 2 bag, 49 GB) + `config/` | Hesai LiDAR (109k pts/scan) + 100 Hz IMU + GNSS, 27 min vehicle drive | glim (via `glim_rosbag`) |
...
| glim | ✅ | **Korea_drive** ROS 2 bag via `glim_rosbag` (16,367 poses, 11.06 km, ATE 5.3 m 2D / 16.5 m 3D vs GNSS; GPU odometry 1.5-1.8x faster and ~4x more accurate than CPU) and KITTI 04 / 00 via the `glim_kitti` driver (ATE 2.60 m over 394 m) | any LiDAR+IMU ROS 2 bag -- image ships ROS 2 Jazzy + glim_ros2 v1.0.0. Loop closure on Korea_drive does NOT close (51 m vertical); see its NOTES.md |
