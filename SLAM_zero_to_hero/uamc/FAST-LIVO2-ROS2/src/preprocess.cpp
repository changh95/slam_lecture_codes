...
void Preprocess::process(const livox_custom_msg::CustomMsg::SharedPtr &msg, PointCloudXYZI::Ptr &pcl_out)
{
  avia_handler(msg);
  *pcl_out = pl_surf;
}
...
void Preprocess::avia_handler(const livox_custom_msg::CustomMsg::SharedPtr &msg)
{
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();
...
void Preprocess::mid360_handler(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &msg)
{
  ...
  for (int i = 0; i < plsize; i++)
  {
    ...
    added_pt.z = pt.z;
    added_pt.intensity = pt.intensity;
    added_pt.curvature = float(pt.timestamp * 1e-9 - msg_header_time) * 1e3; // curvature unit: ms
    pl_surf.points.push_back(added_pt);
  }
}
