...
public:
  ...
  double rs_tr_readout    = 0.0;     // total top-to-bottom frame readout time, seconds
  double rs_anchor_factor = 0.5;     // 0.0=stamp at row 0, 0.5=middle row, 1.0=last row
  bool   rs_debug_print   = false;   // print per-iteration VIO mean residual for td tuning
  V3D    rs_omega_b       = V3D::Zero();   // body-frame angular velocity at frame time (bias-corrected)
  V3D    rs_accel_w       = V3D::Zero();   // world-frame linear accel at frame time (gravity already added)

  // --- online td (img_time_offset) refinement: decoupled 1D Gauss-Newton ---
  bool   td_estimate_enable = false;
  double td_step_alpha      = 0.1;       // learning rate per frame
  double td_step_cap        = 0.0005;    // max absolute step per frame, seconds (0.5 ms)
  double td_lambda          = 100.0;     // Tikhonov damping
  double td_min_jac_sq      = 1.0;       // minimum accumulated jac^2 to trust the step
  double td_step_pending    = 0.0;       // step computed this frame, to be consumed by LIVMapper
  double td_last_grad_sum   = 0.0;       // diagnostic
  double td_last_jac_sq_sum = 0.0;       // diagnostic
