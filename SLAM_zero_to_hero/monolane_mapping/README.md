...
Don't go hunting for a second link — there isn't one. Every revision of upstream's readme has carried exactly two dataset URLs, the OneDrive above and its Baidu mirror, and both are the same `rosbag` + `test` archive. Three people filed the same surprise ([#8](https://github.com/HKUST-Aerial-Robotics/MonoLaneMapping/issues/8), [#12](https://github.com/HKUST-Aerial-Robotics/MonoLaneMapping/issues/12), [#30](https://github.com/HKUST-Aerial-Robotics/MonoLaneMapping/issues/30)) and the author's answer is consistent — *"If you want to reproduce the results, you should download the original complete OpenLane dataset"*, and on `annotation_dir` specifically ([#3](https://github.com/HKUST-Aerial-Robotics/MonoLaneMapping/issues/3)) *"It contains extrinsic parameters and lane marking labels of the original dataset OpenLane. It's necessary to run the code and evaluate the method."* The trimmed archive is deliberate: he changed the code so `demo_mapping.py` runs without it.

Waymo's own tfrecords are the worse route even with access — ~1 GB per 20-second segment to keep one camera, you write the extractor, and you still need the OpenLane jsons for calibration. The GCS bucket answers `403` on individual objects and `401` on listing, so it is authenticated-only either way.

One shortcut once you do have the jsons: per [`docs/annotation.md`](https://github.com/HKUST-Aerial-Robotics/MonoLaneMapping/blob/main/docs/annotation.md) every lane carries a `uv` field, so the **ground-truth** lanes arrive pre-projected into image coordinates and need no intrinsic. The optimised map still has to be projected — nothing precomputes pixels for it.
